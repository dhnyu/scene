"""End-to-end M1.4.2 Road Adapter workflow."""
...
from scene.roads.reader import RoadReader, find_latest_canonical_manifest
from scene.roads.reporting import write_road_report
...
def run_roads(
    ...
    *,
    ...
    """Create and serialize road datasets from M1.3 outputs only."""
    ...
    logger = configure_logging(
        ...
        / f"{metadata.run_id}_m1_4_2_road_adapter.jsonl",
        metadata.run_id,
        level=log_level,
    )
    logger.info("M1.4.2 Road Adapter started")
    source_paths = _source_paths(config)
    source_stats_before = _stats(source_paths)

    canonical_input = RoadReader(
        schema, config.paths.output_root
    ).read(manifest_path)
    canonical_paths = (
        canonical_input.manifest_path,
        Path(canonical_input.link_provenance.canonical_frame_path),
        Path(canonical_input.node_provenance.canonical_frame_path),
    )
    canonical_stats_before = _stats(canonical_paths)
    result = RoadAdapter(RoadValidator(schema)).adapt(canonical_input)
    logger.info(
        "Road dataset validation completed",
        extra={
            "road_link_feature_count": result.links.feature_count,
            "road_node_feature_count": result.nodes.feature_count,
    ...
    artifacts = RoadSerializer().serialize(
        ...
        result.nodes,
        result.validation,
        config.paths.output_root / "roads" / metadata.run_id,
    ...
    return {
        ...
        "link_attribute_parquet": str(artifacts.link_attribute_parquet),
        "markdown_report": str(reports.markdown),
        "metadata_json": str(artifacts.metadata_json),
        "node_attribute_parquet": str(artifacts.node_attribute_parquet),
